...
public class ExpensesController(
    IQueryHandler<GetExpensesQuery, PagedExpenseResponse> getExpensesHandler,
    ...
    ICommandHandler<UpdateExpenseCommand, bool> updateExpenseHandler,
    ICommandHandler<DeleteExpenseCommand, bool> deleteExpenseHandler,
    ...
    ICommandHandler<UpdateExpenseCategoryCommand, bool> updateCategoryHandler,
    ICommandHandler<DeleteExpenseCategoryCommand, bool> deleteCategoryHandler) : Controller
{
    ...
    {
        Result<PagedExpenseResponse> result = await getExpensesHandler.Handle(
    ...
    public async Task<ActionResult> Create([FromBody] CreateExpenseModel model, CancellationToken cancellationToken)
    {
        ...
        if (!result.IsSuccess)
        {
            return Json(ToastResult.ErrorResult(result.Error.Description, "المصروفات"));
        }
    ...
    public async Task<ActionResult> Update([FromBody] EditExpenseModel model, CancellationToken cancellationToken)
    {
        ...
        Result<bool> result = await updateExpenseHandler.Handle(command, cancellationToken);
    ...
    {
        Result<bool> result = await deleteExpenseHandler.Handle(new DeleteExpenseCommand(id), cancellationToken);
    ...
    public async Task<ActionResult> CreateCategory([FromBody] ExpenseCategoryModel model, CancellationToken cancellationToken)
    {
        ...
        if (!result.IsSuccess)
        {
            return Json(ToastResult.ErrorResult(result.Error.Description, "تصنيفات المصروفات"));
        }
    ...
    public async Task<ActionResult> UpdateCategory([FromBody] UpdateExpenseCategoryModel model, CancellationToken cancellationToken)
    {
        ...
        Result<bool> result = await updateCategoryHandler.Handle(new UpdateExpenseCategoryCommand(model.Id, model.Name), cancellationToken);
    ...
    {
        Result<bool> result = await deleteCategoryHandler.Handle(new DeleteExpenseCategoryCommand(id), cancellationToken);

...
import type { CustomerPurchaseInvoiceResponse } from './purchases-api.service';
...
/**
 * P3.9 — customer gold purchase invoice detail. Shows the header (number, seller, buying
 * employee, payment method, seller bank account) and the invoice lines (karat / weight /
 * 21K-equivalent / price / gold value), plus totals. Receives the row from the list for
 * instant render, then refreshes it from `GET /{id}` while open so the data is always current.
 */
@Component({
  ...
  template: `
    <app-dialog
      [open]="open()"
      [title]="displayInvoice()?.invoiceNumber ?? 'فاتورة شراء ذهب'"
        ...
        <div class="space-y-4">
          <div class="grid grid-cols-2 gap-3 sm:grid-cols-3">
            ...
            <div>
              ...
              <p class="mt-0.5 text-sm text-gray-900 data-mono" dir="ltr">
                {{ invoice.sellerIdNumber ?? '—' }}
              </p>
            </div>
            <div>
              <p class="text-xs text-gray-500">سنة الميلاد</p>
              <p class="mt-0.5 text-sm text-gray-900 data-mono" dir="ltr">
                {{ invoice.sellerYearOfBirth ?? '—' }}
              </p>
            </div>
            <div>
              <p class="text-xs text-gray-500">العنوان</p>
              <p class="mt-0.5 text-sm text-gray-900">{{ invoice.sellerAddress ?? '—' }}</p>
            </div>
            <div>
            ...
            <div>
              <p class="text-xs text-gray-500">الموظف (المشتري)</p>
              <p class="mt-0.5 text-sm text-gray-900">{{ invoice.employeeName ?? '—' }}</p>
            </div>
            <div>
            ...
            <div>
              <p class="text-xs text-gray-500">حساب البائع</p>
              <p class="mt-0.5 text-sm text-gray-900 data-mono" dir="ltr">
                {{ invoice.sellerAccountNumber ?? '—' }}
              </p>
            </div>
          </div>
...
export class PurchaseDetailDialog {
  ...
  readonly invoice = input<CustomerPurchaseInvoiceResponse | null>(null);
